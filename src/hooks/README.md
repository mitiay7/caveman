...
- Emits a small per-turn reinforcement reminder when the flag is set to a non-independent mode (`lite`/`smart`/`full`/`ultra`/`wenyan*`)
- Supports: `lite`, `smart`, `full`, `ultra`, `wenyan`, `wenyan-lite`, `wenyan-full`, `wenyan-ultra`, `commit`, `review`, `compress`
