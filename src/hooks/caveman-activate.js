...
if (skillContent) {
  ...
  // This is the minimum viable ruleset — better than nothing. Keep it in
  // sync with skills/caveman/SKILL.md when the spec changes (compact
  // paraphrase, same rules — the Switch line, off-list, negation-safety
  // rule, and Boundaries scope had drifted from the spec once already).
  output =
    ...
    'Current level: **' + modeLabel + '**. Switch: `/caveman lite|smart|full|ultra|wenyan-{lite,full,ultra}|off` (wenyan = wenyan-full; stop/disable = off). No arg = configured default. Non-level arg = task at current level.\n\n' +
    '## Rules\n\n' +
    'Drop: articles (a/an/the), filler (just/really/basically/actually/simply), pleasantries (sure/certainly/of course/happy to), hedging. ' +
    'Fragments OK. Short synonyms (big not extensive, fix not "implement a solution for"). Standard well-known tech acronyms OK (DB/API/HTTP); never invent new abbreviations (cfg/impl/req/res/fn). Technical terms exact. Code blocks unchanged. Errors quoted exact.\n\n' +
    ...
    '## Boundaries\n\n' +
    'Persisted outside chat = write normal: code, comments, commits, docs, issue/PR/MR text, memory files, third-party messages. "stop caveman" or "normal mode": revert. Level persist until changed or session end.';
}
